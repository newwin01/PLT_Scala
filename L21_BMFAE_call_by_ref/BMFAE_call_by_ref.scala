...
def interp(expr: Expr, ds: DefrdSub, st: Store): ValueStore = expr match {
  ...
  case App(f,a) => a match {
    case Id(name) => interp(f, ds, st) match {
      case vs(f_value, st1) =>
        val a = lookup(name, ds)
        f_value match {
          case ClosureV(param, body, ds) =>
            interp(body, ASub(param, a, ds), st1)
        }
    }
    case _ => interp(f, ds, st) match {
...
@main def run(): Unit =
  ...
  println(run("{with {a 3} {seqn {{fun {x} {setvar x 5}} a} a}}", MtSub, MtSto))
  //println(run("{with {a {newbox 3}} {seqn {{fun {x} {setbox x 5}} a} {openbox a}}}", MtSub, MtSto))
  println(run("{with {swap {fun {x} {fun {y} {with {z x} {seqn {setvar x y} {setvar y z}}}}}} {with {a 10} {with {b 20} {seqn {{swap a} b} a}}}}", MtSub, MtSto))
  println(run("{with {swap {fun {x} {fun {y} {with {z x} {seqn {setvar x y} {setvar y z}}}}}} {with {a 10} {with {b 20} {seqn {{swap a} b} b}}}}", MtSub, MtSto))
